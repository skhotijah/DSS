	...
	<div>
	  ...
	  <div class="tab-content">
		...
		?>
			...
			Karyawan yang berhak diterima di Markas Hobby adalah
...
mySQL_select_db("spk_saw");

$query = 'SELECT max(nama_alternatif) as hasil_alternatif FROM alternatif GROUP BY hasil_alternatif ';//varchar 15 entry
$result = mysql_query($query) or die(mysql_error());
while($row = mysql_fetch_array($result)){
    $maxhasil = $row['hasil_alternatif'];
}
echo $maxhasil;

	
		?>
		dengan hasil analisa Simple Additive Weighting (SAW):
		
		
		<?php

		$query = 'SELECT MAX(hasil_alternatif) as hasil_alternatif FROM alternatif ';//varchar 15 entry
$result = mysql_query($query) or die(mysql_error());
while($row = mysql_fetch_array($result)){
    $maxhasil = $row['hasil_alternatif'];
}
echo $maxhasil;
	...
	<footer class="text-center">SKRIPSI &copy; 2016 Siti Khotijah</footer>
	</div>
