...
class PDF extends FPDF{
	...
	function Header(){
	    ...
	    $this->Cell(130);
	    $this->Cell(30,15,'LAPORAN SISTEM PENDUKUNG KEPUTUSAN',0,0,'C');
	    $this->Ln(15);
	}
...
while ($row2x = $stmt2x->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(40,7,$row2x['nama_kriteria'],1,0,'L');
}
...
while ($row1x = $stmt1x->fetch(PDO::FETCH_ASSOC)){
	...
	while ($rowrx = $stmtrx->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(40,7,$rowrx['nilai_rangking'],1,0,'L');
	}
	$pdf->ln();
}
...
while ($row2y = $stmt2y->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(40,7,$row2y['nama_kriteria'],1,0,'L');
}
...
while ($row1y = $stmt1y->fetch(PDO::FETCH_ASSOC)){
	...
	while ($rowry = $stmtry->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(40,7,number_format($rowry['nilai_normalisasi'], 1, '.', ','),1,0,'L');
	}
	$pdf->ln();
}
$pdf->Cell(40,7,'Bobot',1,0,'L');
while ($row2yx = $stmt2yx->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(40,7,$row2yx['bobot_kriteria'],1,0,'L');
}
$pdf->ln();
...
while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(35,7,$row1['nama_alternatif'],1,0,'L');
	$stmtr = $pro->readR($row1['id_alternatif']);
	while ($rowr = $stmtr->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(35,7,number_format($rowr['bobot_normalisasi'], 2, '.', ','),1,0,'L');
	}
